...
locals {
  default_node_pool_zones = setsubtract(local.zones, local.restrictions)
  filtered_vms = [
    for item in data.azapi_resource_list.example.output.value :
    item if(item.resourceType == "virtualMachines" && item.name == "Standard_D4d_v5")
  ]
  restrictions = try(local.filtered_vms[0].restrictions[0].restrictionInfo.zones, [])
  zones        = local.filtered_vms[0].locationInfo[0].zones
}

locals {
  filtered_vms_by_pool = {
    for pool_name, pool in var.node_pools : pool_name => [
      for item in data.azapi_resource_list.example.output.value :
      item if(item.resourceType == "virtualMachines" && item.name == pool.vm_size)
    ]
  }
  my_node_pool_zones_by_pool = {
    for pool_name, pool in var.node_pools : pool_name => setsubtract(
      local.filtered_vms_by_pool[pool_name][0].locationInfo[0].zones,
      try(local.filtered_vms_by_pool[pool_name][0].restrictions[0].restrictionInfo.zones, [])
    )
  }
